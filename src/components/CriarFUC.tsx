...
const CriarFUC = () => {
    ...
    const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (e) => {
            ...
            const camposExtraidos: Campo[] = [];
            let currentCampo: Partial<Campo> = {};
            let bufferDescricao: string[] = [];

            const tipoAutomatico = (descricao: string): Campo['tipo'] => {
                const desc = descricao.toLowerCase();
                if (desc.includes('numéric')) return 'numerico';
                if (desc.includes('tabela') || desc.includes('tipologia')) return 'tabela';
                return 'texto';
            };
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                // Detecção de nova secção
                const matchCampo = line.match(/^(\d+(\.\d+)*)(\.|\))?\s*(.+)/);
                if (matchCampo) {
                    // Guardar campo anterior
                    if (currentCampo.titulo && bufferDescricao.length > 0) {
                        camposExtraidos.push({
                            id: `campo_${Date.now()}_${camposExtraidos.length}`,
                            titulo: currentCampo.titulo,
                            descricao: bufferDescricao.join('\n').trim(),
                            tipo: currentCampo.tipo ?? 'texto',
                            maxCaracteres: 1000
                        });
                    }

                    currentCampo = { titulo: matchCampo[1] + '. ' + matchCampo[4] };
                    bufferDescricao = [];
                } else if (/^campo /i.test(line)) {
                    // Detecção de tipo automático
                    currentCampo.tipo = tipoAutomatico(line);
                    bufferDescricao.push(line);
                } else {
                    bufferDescricao.push(line);
                }
            }
            ...
            setTipo(''); // O utilizador insere depois
